...
def rtn_recv(se: 'SymbolicExecutor', pstate: 'ProcessState'):
    logger.debug('recv hooked')
    return 0;

def rtn_send(se: 'SymbolicExecutor', pstate: 'ProcessState'):
    logger.debug('send hooked')
    return 0;

def rtn_close(se: 'SymbolicExecutor', pstate: 'ProcessState'):
    logger.debug('close hooked')
    return 0;
...
def rtn_socket(se: 'SymbolicExecutor', pstate: 'ProcessState'):
    logger.debug('socket hooked')
    return 0;

def rtn__ITM_deregisterTMCloneTable(se: 'SymbolicExecutor', pstate: 'ProcessState'):
    logger.debug('_ITM_deregisterTMCloneTable hooked')
    return 0;

def rtn_htons(se: 'SymbolicExecutor', pstate: 'ProcessState'):
    logger.debug('htons hooked')
    return 0;

def rtn___gmon_start__(se: 'SymbolicExecutor', pstate: 'ProcessState'):
    logger.debug('__gmon_start__ hooked')
    return 0;


def rtn__ITM_registerTMCloneTable(se: 'SymbolicExecutor', pstate: 'ProcessState'):
...
SUPPORTED_ROUTINES = {
    ...
    'send':                    rtn_send,
    'close':                   rtn_close,
    'inet_pton':               rtn_inet_pton,
    '__isoc99_scanf':          rtn___isoc99_scanf,
    'connect':                 rtn_connect,
    'socket':                  rtn_socket,
    '_ITM_deregisterTMCloneTable': rtn__ITM_deregisterTMCloneTable,
